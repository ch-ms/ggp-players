...
public class ChmsGoalProximitySearchPlayer extends SampleGamer {
	
	// Roles of players
	private Role opponent;
	private Role me;
	private static final Integer LIMIT = 3; // limit of search depth
	...
	private Integer getMaxScore(MachineState state, Integer level) throws GoalDefinitionException, MoveDefinitionException, TransitionDefinitionException {
		if(getStateMachine().isTerminal(state)){
			...
			return goalProximityEval(state);
		}else{
	 ...
	 * @throws GoalDefinitionException 
	 */
	private Integer goalProximityEval(MachineState state) throws MoveDefinitionException, GoalDefinitionException{
		return getStateMachine().getGoal(state, me);
	}
